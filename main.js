...
function generateReceipt() {
    ...
    const transactionData = {
        ...
        merchantId: 'ID1023304212781'
    };
    
    const qrisData = formatQRISData(transactionData);
...
// Format data sesuai spesifikasi QRIS Nasional
function formatQRISData(transactionData) {
    // Implementasi sesuai spesifikasi QRIS
    // Ini adalah contoh sederhana, sesuaikan dengan kebutuhan sebenarnya
    const amount = Math.round(transactionData.amount).toString().padStart(10, '0');
    return `00020101021126680014ID.ORCQR.WWW0118936001140306${amount}5204581253033605802ID5914MONASCHO JEMBER6007JEMBER61056850062070703A016304`;
}
...
function showReceipt() {
    ...
    // Generate QR code setelah konten struk dimuat
    setTimeout(() => {
        const subtotal = orders.reduce((sum, order) => sum + order.total, 0);
        discount = parseInt(discountInput.value) || 0;
        shippingCost = parseInt(shippingInput.value) || 0;
        ...
        const transactionData = {
            amount: total,
            transactionId: `MNSC-${Date.now()}`,
            merchantId: 'ID1023304212781'
        };
        
        const qrisData = formatQRISData(transactionData);
        generateQRCode(qrisData, 'qrisImage');
    }, 100);
